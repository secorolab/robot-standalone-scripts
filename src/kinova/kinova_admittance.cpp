...
int main(int argc, char ** argv)
{
  ...
  KDL::ChainIdSolver_RNE id_solver(chain, KDL::Vector(-9.5, -1.19, 1.11));
  ...
  try {
    ...
    id_solver.CartToJnt(q, zeroArray, zeroArray, f_ext, gtau);

    for (unsigned int i = 0; i < NUM_JOINTS; i++)
    {
      base_command.mutable_actuators(i)->set_torque_joint(gtau(i));
  ...
  while(true) {
    ...
    for (unsigned int i = 0; i < NUM_JOINTS; i++)
    {
      ...
      etau(i) = mtau(i) + ctau(i);

      if (fabs(etau(i)) > 1.0) set_qref = true;
    }
